...
module.exports = async function (client) {
    try {
        ...
        */
        const commandFolders = fs.readdirSync(__dirname + '/../commands/message/');
    
        for (const folder of commandFolders) {
            const commandFiles = fs.readdirSync(__dirname + `/../commands/message/${folder}`).filter(file => file.endsWith('.js'));
            for (const file of commandFiles) {
                // https://discordjs.guide/creating-your-bot/command-handling.html#reading-command-files
                const command = require(__dirname + `/../commands/message/${folder}/${file}`);
